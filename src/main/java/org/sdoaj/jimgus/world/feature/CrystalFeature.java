...
public class CrystalFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        List<Vec3f> spline = SplineHelper.makeSpline(0, 0, 0, 0, 60, 0, 5);
//        SplineHelper.offsetPoints(spline, () -> MathHelper.lerp(random.nextFloat(), -1, 1), 3, 0, 3);
        SplineHelper.offsetPoints(spline, () -> (float) random.nextGaussian(), 3, 0, 3);
        SDF sdf = SplineHelper.SplineSDFBuilder.from(spline).radius(10, 2).build().setBlock(Util.pickRandom(random, blocks));
        sdf.fill(world, pos);
